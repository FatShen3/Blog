...
flushCallbacks就是把callbacks一个一个取出来挨个执行，但是这里区分使用了macro task(setImmediate(IE独有)、MessageChannel的、 setTimeout按优先级依次使用)和micro task(原生Promise、setTimeout按优先级依次使用)。
